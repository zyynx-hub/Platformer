# BossPhase3State.gd
# Combined phase: alternates charge and dive attacks with faster timing.
# Charge speed and rise/dive duration scaled by phase3_speed_multiplier.
# All timing from @export vars on ExodiaKarimBoss.

extends BossState

enum SubState { APPROACH, TELEGRAPH_CHARGE, CHARGE, RISE, TELEGRAPH_DIVE, DIVE, RECOVERY }

const OFFSCREEN_TOP_Y: float = -185.0

var _sub_state: SubState = SubState.APPROACH
var _telegraph_timer: float = 0.0
var _recovery_timer: float = 0.0
var _blink_timer: float = 0.0
var _blink_on: bool = false
var _charge_direction: float = 1.0
var _next_is_dive: bool = false
var _active_tween: Tween = null


func enter() -> void:
	_next_is_dive = false
	_blink_on = false
	boss.show_danger_indicator()
	boss.set_hitbox_active(true)
	boss.global_position.y = boss.arena_ground_y
	_sub_state = SubState.APPROACH


func exit() -> void:
	_kill_tween()
	_hide_telegraph()
	boss.hide_danger_indicator()
	boss.hide_stomp_indicator()
	boss.set_stunnable(false)
	boss.set_composite_flash(0.0)
	boss.restore_head()
	boss.set_hitbox_active(true)
	# Ensure boss is back on-screen
	boss.global_position.y = boss.arena_ground_y


func physics_update(delta: float) -> void:
	match _sub_state:
		SubState.APPROACH:
			_do_approach(delta)
		SubState.TELEGRAPH_CHARGE:
			_do_telegraph_charge(delta)
		SubState.TELEGRAPH_DIVE:
			_do_telegraph_dive(delta)
		SubState.RECOVERY:
			_do_recovery(delta)


# --- Next attack selector ---

func _start_next_attack() -> void:
	if _next_is_dive:
		_start_rise()
	else:
		_sub_state = SubState.APPROACH
		boss.set_hitbox_active(true)


# --- Charge attack (same as Phase 1, faster) ---

func _do_approach(delta: float) -> void:
	var player = boss.get_player_ref()
	if not player:
		return
	var spd: float = boss.phase1_walk_speed * boss.phase3_speed_multiplier
	var dir := signf(player.global_position.x - boss.global_position.x)
	if dir == 0.0:
		dir = 1.0
	boss.global_position.x += dir * spd * delta
	boss.global_position.x = clampf(boss.global_position.x, boss.arena_left, boss.arena_right)
	boss.global_position.y = boss.arena_ground_y

	var dist := absf(player.global_position.x - boss.global_position.x)
	if dist <= boss.phase1_charge_range:
		_start_telegraph_charge()


func _start_telegraph_charge() -> void:
	_sub_state = SubState.TELEGRAPH_CHARGE
	_telegraph_timer = boss.phase3_telegraph_duration
	var player = boss.get_player_ref()
	if player:
		_charge_direction = signf(player.global_position.x - boss.global_position.x)
	if _charge_direction == 0.0:
		_charge_direction = 1.0
	boss.play_rumble()


func _do_telegraph_charge(delta: float) -> void:
	_telegraph_timer -= delta
	# Faster flash in Phase 3
	var flash: float = 0.5 if fmod(_telegraph_timer, 0.15) > 0.075 else 0.0
	boss.set_composite_flash(flash)
	if _telegraph_timer <= 0.0:
		boss.set_composite_flash(0.0)
		_start_charge()


func _start_charge() -> void:
	_sub_state = SubState.CHARGE
	_kill_tween()
	var target_x: float = boss.arena_right if _charge_direction > 0.0 else boss.arena_left
	var dist: float = absf(target_x - boss.global_position.x)
	var spd: float = boss.phase1_charge_speed * boss.phase3_speed_multiplier
	var duration: float = dist / spd
	if duration < 0.05:
		duration = 0.05
	_active_tween = boss.create_tween()
	_active_tween.set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_QUAD)
	_active_tween.tween_property(boss, "global_position:x", target_x, duration)
	_active_tween.tween_callback(_on_charge_hit_wall)


func _on_charge_hit_wall() -> void:
	EventBus.camera_shake_requested.emit(0.3, 5.0)
	boss.play_impact()
	_enter_recovery()


# --- Dive attack (same as Phase 2, faster) ---

func _start_rise() -> void:
	_sub_state = SubState.RISE
	_kill_tween()
	boss.set_hitbox_active(false)
	var rise_dur: float = boss.phase2_rise_duration / boss.phase3_speed_multiplier
	_active_tween = boss.create_tween()
	_active_tween.set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_QUAD)
	_active_tween.tween_property(boss, "global_position:y", OFFSCREEN_TOP_Y, rise_dur)
	_active_tween.tween_callback(_begin_telegraph_dive)


func _begin_telegraph_dive() -> void:
	_sub_state = SubState.TELEGRAPH_DIVE
	var player = boss.get_player_ref()
	if player:
		boss.global_position.x = clampf(player.global_position.x, boss.arena_left, boss.arena_right)
	else:
		boss.global_position.x = boss.arena_center_x
	_telegraph_timer = boss.phase3_telegraph_duration
	_show_telegraph()


func _do_telegraph_dive(delta: float) -> void:
	_telegraph_timer -= delta
	if _telegraph_timer <= 0.0:
		_hide_telegraph()
		_start_dive()


func _start_dive() -> void:
	_sub_state = SubState.DIVE
	_kill_tween()
	boss.set_hitbox_active(true)
	var dive_dur: float = boss.phase2_dive_duration / boss.phase3_speed_multiplier
	_active_tween = boss.create_tween()
	_active_tween.set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_QUAD)
	_active_tween.tween_property(boss, "global_position:y", boss.arena_ground_y, dive_dur)
	_active_tween.tween_callback(_on_dive_impact)


func _on_dive_impact() -> void:
	EventBus.camera_shake_requested.emit(0.4, 6.0)
	boss.play_impact()
	_enter_recovery()


# --- Recovery (shared by charge and dive) ---

func _enter_recovery() -> void:
	_sub_state = SubState.RECOVERY
	_recovery_timer = boss.phase3_recovery_duration
	_blink_timer = 0.0
	_blink_on = false
	boss.set_stunnable(true)
	boss.show_stomp_indicator()
	boss.set_hitbox_active(false)
	boss.droop_head()


func _do_recovery(delta: float) -> void:
	_recovery_timer -= delta
	if _recovery_timer <= boss.recovery_blink_before_end:
		_blink_timer -= delta
		if _blink_timer <= 0.0:
			_blink_on = not _blink_on
			_blink_timer = 0.15
			boss.set_composite_flash(0.5 if _blink_on else 0.0)
	if _recovery_timer <= 0.0 and boss._stunnable:
		boss.set_stunnable(false)
		boss.hide_stomp_indicator()
		boss.set_composite_flash(0.0)
		boss.restore_head()
		_next_is_dive = not _next_is_dive
		_start_next_attack()


# --- Telegraph visual (dive only) ---

func _show_telegraph() -> void:
	var line = boss.get_telegraph_line()
	if line:
		line.position = Vector2(-2, boss.arena_top_y - boss.global_position.y)
		line.size.y = absf(boss.arena_ground_y - boss.arena_top_y) + 40.0
		line.visible = true


func _hide_telegraph() -> void:
	var line = boss.get_telegraph_line()
	if line:
		line.visible = false


func _kill_tween() -> void:
	if _active_tween and _active_tween.is_valid():
		_active_tween.kill()
		_active_tween = null
