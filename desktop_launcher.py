    ...
    def _write_updater_script(self, target_exe, downloaded_exe, backup_exe):
        try:
            ...
            script_lines = [
                ...
                '  if exist "%TARGET%" (',
                '    set "NEWPID="',
                '    for /f %%P in (\'powershell -NoProfile -ExecutionPolicy Bypass -Command "$p=Start-Process -FilePath $env:TARGET -WorkingDirectory $env:TARGET_DIR -PassThru; if($p){$p.Id}" 2^>nul\') do set "NEWPID=%%P"',
                '    if defined NEWPID (',
                '      echo [Updater] launch pid !NEWPID! attempt %%R>>"%LOG%"',
                "      timeout /t 1 /nobreak >nul",
                '      tasklist /FI "PID eq !NEWPID!" | findstr /I "!NEWPID!" >nul && set "START_OK=1"',
                "    )",
                "  )",
                '  if "!START_OK!"=="1" goto :launch_done',
                "  timeout /t 1 /nobreak >nul",
                ")",
