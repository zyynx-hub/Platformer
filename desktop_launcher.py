    ...
    def _write_updater_script(self, target_exe, downloaded_exe, backup_exe, current_pid, updater_log_path):
        try:
            ...
            script_lines = [
                ...
                'set "START_OK=0"',
                'powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "$ok=$false; for($i=0;$i -lt 12 -and -not $ok;$i++){ try{ $p=Start-Process -FilePath $env:TARGET -WorkingDirectory $env:TARGET_DIR -PassThru -ErrorAction Stop; Start-Sleep -Milliseconds 900; if(Get-Process -Id $p.Id -ErrorAction SilentlyContinue){ $ok=$true } } catch{}; if(-not $ok){ Start-Sleep -Milliseconds 500 } }; if($ok){ exit 0 } else { exit 1 }" >nul 2>&1',
